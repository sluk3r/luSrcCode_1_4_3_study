...
final class DocumentWriter {
    ...
    final void addDocument(String segment, Document doc)
        ...
        */
        ...
        // write postings
        writePostings(postings, segment); //wangxc 怎么都要带上这个segment？
    ...
    private final void invertDocument(Document doc)
        ...
        while (fields.hasMoreElements()) {
            ...
            int fieldNumber = fieldInfos.fieldNumber(fieldName); //wangxc 有可能是-1，

            int length = fieldLengths[fieldNumber];     // length of field
            int position = fieldPositions[fieldNumber]; // position in field   //wangxc 这个Postiong的原始值可能会是多少？ 没找到是什么时候赋值。  是不是都是0？ 后来会再把这个值写回到数组里？

            if (field.isIndexed()) {
                if (!field.isTokenized()) {          // un-tokenized field
                    addPosition(fieldName, field.stringValue(), position++);
                    length++;
                } else { //wangxc 分词时的情况只是一个分支。
                    ...
                    try {
                        for (Token t = stream.next(); t != null; t = stream.next()) {
                            ...
                            addPosition(fieldName, t.termText(), position++);//wangxc 这个Postion是不是跟Posting有关？看到一个这样的解释： (bookkeeping) a listing on the company's records。这个意思应该接近于Lucene里Posting的意义。
    ...
    //wangxc 跟Posting代表了核心业务一样，这个方法应该是代表了操作上的核心。
    private final void addPosition(String field, String text, int position) {
        termBuffer.set(field, text);
        Posting ti = (Posting) postingTable.get(termBuffer);
        if (ti != null) {                  // word seen before
            int freq = ti.freq;
            if (ti.positions.length == freq) {      // positions array is full      //wangxc 这个长度应该总是等于freq的吧？ 2014-5-10 14:53:27 Problem
    ...
    private final void writePostings(Posting[] postings, String segment)
        ...
        try {
            ...
            TermInfo ti = new TermInfo();//wangxc 这个信息只有在保存时，才组建。
...
final class Posting {                  // info about a Term in a doc //wangxc 这个应该是自己实现时的核心类。
    Term term;                      // the Term
